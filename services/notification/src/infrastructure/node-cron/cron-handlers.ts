...
import { RMQ_QUEUE_EMAIL_VERIFICATION, RMQ_QUEUE_EMAIL_WELCOME } from "../rabbitmq/rabbitmq-config.js";
...
export async function cronInboxEventHandler() {
  ...
  for (const event of unhandledEvents) {
    switch (event.queue) {
      case RMQ_QUEUE_EMAIL_VERIFICATION:
        queueProduce(queueEmailVerification, event);
        break;

      case RMQ_QUEUE_EMAIL_WELCOME:
