...
import { throwCONFLICT, throwNOT_FOUND, throwUNAUTHORIZED } from "../utils/app-error.js";
import { InspectTokenDto, InspectTokenRequest } from "../schemas/auth/inspect-token.request.js";
import { RenewTokenDto, RenewTokenRequest } from "../schemas/auth/renew-token.request.js";
...
async function login(payload: LoginRequest): Promise<AuthDto> {
  ...
  const user = await prisma.user.findUnique({
    where: { username },
    include: { roles: { select: { name: true } }, claims: { select: { name: true } } },
  });

  if (!user) throwNOT_FOUND("User");

  const isPasswordMatch = await BcryptUtil.verifyPassword(password, user.password);
...
async function register(payload: RegisterRequest): Promise<AuthDto> {
  ...
  const { accessToken, refreshToken } = await JwtUtil.generateTokens(app, newUser);
...
async function renewToken(payload: RenewTokenRequest): Promise<RenewTokenDto> {
  ...
  const user = await prisma.user.findUnique({
    ...
    include: { roles: { select: { name: true, id: false } }, claims: { select: { name: true } } },
  });
  if (!user) throwUNAUTHORIZED();

  const { accessToken, refreshToken: newRefreshToken } = await JwtUtil.generateTokens(app, user);
