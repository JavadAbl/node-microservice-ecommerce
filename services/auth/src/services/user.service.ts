...
import { SetUserPermissionDto } from "../schemas/user/request-schema/set-user-permission.schema.js";
...
async function setUserRole(userId: number, payload: SetUserRoleDto): Promise<void> {
  const { role } = payload;
  await userRep.findAndCheckExistsBy(
    { where: { id: userId } },
    { hasThrow: true, field: "id", value: userId },
  );
